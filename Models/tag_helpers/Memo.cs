...
namespace SaYMemos.Models.tag_helpers
{
    ...
    public class Memo : TagHelper
    {
        ...
        public override void Process(TagHelperContext context, TagHelperOutput output)
        {
            ...
            //if(MemoContent.commentsAllowed){}

            //content.Append("<div class='memo-interactions'>");

            //content.AppendFormat("<span class='memo-likes'{0} {1}</span>", LikeIcon().EncodeHtml(), MemoContent.LikesCount.EncodeHtml());

            //content.AppendFormat("<span class='memo-comments'>{0} {1}</span>", CommentIcon().EncodeHtml(), MemoContent.CommentsCount.EncodeHtml());
            //content.Append("</div>");
